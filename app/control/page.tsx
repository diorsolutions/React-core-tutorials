...
import { Eye, EyeOff, Shield, AlertTriangle, Plus, Edit, Trash2, Save, X } from "lucide-react"
...
export default function AdminControlPage() {
  ...
  const checkAuthStatus = () => {
    ...
    if (session) {
      ...
      const now = Date.now()
      if (sessionData.isAuthenticated && now - sessionData.loginTime < SESSION_DURATION) {
  ...
  const checkBlockStatus = () => {
    ...
    if (attempts) {
      ...
      const recentAttempts = attemptData.filter(
        (attempt) => attempt.ip === clientIP && Date.now() - attempt.timestamp < BLOCK_DURATION,
      )

      if (recentAttempts.length >= MAX_ATTEMPTS) {
        const oldestAttempt = recentAttempts[0]
        const timeRemaining = BLOCK_DURATION - (Date.now() - oldestAttempt.timestamp)
        if (timeRemaining > 0) {
          setIsBlocked(true)
  ...
  const recordLoginAttempt = () => {
    ...
    const recentAttempts = attemptData.filter((attempt) => Date.now() - attempt.timestamp < BLOCK_DURATION)

    localStorage.setItem("login-attempts", JSON.stringify(recentAttempts))

    if (recentAttempts.filter((a) => a.ip === clientIP).length >= MAX_ATTEMPTS) {
      setIsBlocked(true)
  ...
  const handleSaveProduct = () => {
    ...
    // Basic validation
    if (!editingProduct.name.en || !editingProduct.price || editingProduct.stock < 0) {
      toast({
    ...
    setTimeout(() => {
      ...
      if (success) {
        ...
        toast({
          ...
          description: `Product has been ${isNewProduct ? "added" : "updated"} successfully`,
        })
      ...
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <Card className="w-full max-w-md">
          <CardHeader className="text-center">
            ...
            <CardTitle className="text-2xl font-bold text-[#302dbb]">Admin Control Panel</CardTitle>
          </CardHeader>
          <CardContent>
            {isBlocked ? (
              <Alert className="mb-4">
                ...
                <AlertDescription>
                  Access blocked due to too many failed attempts. Try again in {formatTime(blockTimeRemaining)}.
                </AlertDescription>
              </Alert>
            ) : (
              <form onSubmit={handleLogin} className="space-y-4">
                ...
                <Button type="submit" className="w-full bg-[#302dbb] hover:bg-[#302dbb]/90" disabled={isBlocked}>
                  Sign In
                </Button>
              </form>
            )}
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-xl font-bold text-[#302dbb]">Admin Control Panel</h1>
            </div>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-6">
            ...
            <Button onClick={handleAddProduct} className="bg-[#302dbb] hover:bg-[#302dbb]/90">
            ...
            {products.map((product) => (
              <Card key={product.id} className="overflow-hidden">
                <div className="relative">
                  ...
                  />
                  {product.popular && <Badge className="absolute top-2 right-2 bg-[#302dbb]">Popular</Badge>}
                </div>
                <CardContent className="p-4">
                  <div className="space-y-2">
                    <h3 className="font-semibold text-lg">{product.name.en}</h3>
                    <p className="text-sm text-muted-foreground line-clamp-2">{product.description.en}</p>
                    <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-2 mt-4">
                    <Button onClick={() => handleEditProduct(product)} variant="outline" size="sm" className="flex-1">
      ...
      <Dialog open={showProductDialog} onOpenChange={setShowProductDialog}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editingProduct?.id.startsWith("product-") ? "Add New Product" : "Edit Product"}</DialogTitle>
          </DialogHeader>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 gap-4">
                  <div>
                    <Label htmlFor="name-uz">Product Name</Label>
                    <Input
                      id="name-uz"
                  ...
                  <div>
                    <Label htmlFor="description-uz">Description</Label>
                    <Textarea
                      id="description-uz"
                      ...
                      placeholder="Product description in Uzbek"
                      rows={3}
                    />
                  </div>
                </div>
              ...
              {/* Russian Fields */}
              <div className="space-y-4">
                <h3 className="font-semibold text-lg">Russian</h3>
                <div className="grid grid-cols-1 gap-4">
                  <div>
                    <Label htmlFor="name-ru">Product Name</Label>
                    <Input
                      id="name-ru"
                  ...
                  <div>
                    <Label htmlFor="description-ru">Description</Label>
                    <Textarea
                      id="description-ru"
              ...
              <div>
                <ImageUpload
                  ...
                  label="Product Image"
                />
                <p className="text-xs text-muted-foreground mt-1">Leave empty to use placeholder image</p>
              </div>
              ...
              <div className="flex gap-3 pt-4">
                ...
                <Button onClick={() => setShowProductDialog(false)} variant="outline" disabled={isLoading}>
